...
impl ConnectCtx {
    fn new(src: &Block, src_port: &PortId, src_output: &StreamOutput, dst: &Block, dst_port: &PortId, dst_input: &StreamInput) -> Self {
        Self {
...
impl Display for ConnectCtx {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "incompatible ports: {}.{}<{}> -> {}.{}<{}>",
            self.src_block, self.src_port, self.src_type, self.dst_block, self.dst_port, self.dst_type
        )
    }
}